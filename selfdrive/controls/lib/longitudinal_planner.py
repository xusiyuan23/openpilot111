...
class LongitudinalPlanner:
  ...
  def update(self, sm):
    if self.param_read_counter % 50 == 0:
      self.read_param()
      self.accel_controller.read_params()
      self.dynamic_endtoend_controller.read_params()
    self.param_read_counter += 1
    # self.mpc.mode = 'blended' if sm['controlsState'].experimentalMode else 'acc'
    self.mpc.mode = self.dynamic_endtoend_controller.set_mpc_mode(self.mpc.mode, self.CP.radarUnavailable, sm['carState'], sm['radarState'].leadOne, sm['modelV2']) if sm['controlsState'].experimentalMode else 'acc'
