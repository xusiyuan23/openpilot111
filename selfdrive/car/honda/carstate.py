...
def get_can_signals(CP, gearbox_msg="GEARBOX"):
  ...
  if CP.carFingerprint in (CAR.ACCORD_2021, CAR.ACCORDH_2021, CAR.CIVIC_BOSCH, CAR.CIVIC_BOSCH_DIESEL, CAR.CRV_HYBRID, CAR.ACURA_RDX_3G):
    signals += [("DRIVERS_DOOR_OPEN", "SCM_FEEDBACK", 1)]
...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam, cp_body):
    ...
    elif self.CP.carFingerprint in (CAR.ACCORD_2021, CAR.ACCORDH_2021, CAR.CIVIC_BOSCH, CAR.CIVIC_BOSCH_DIESEL, CAR.CRV_HYBRID, CAR.ACURA_RDX_3G):
      ret.standstill = cp.vl["ENGINE_DATA"]["XMISSION_SPEED"] < 0.1
      ret.doorOpen = bool(cp.vl["SCM_FEEDBACK"]["DRIVERS_DOOR_OPEN"])
