...
def main(demo=False):
  ...
  params = Params()
  #DP:
  dp_lat_lane_change_assist_speed = int(params.get("dp_lat_lane_change_assist_speed", encoding="utf-8")) * CV.KPH_TO_MS
  dp_auto_lane_change = params.get_bool("dp_auto_lane_change")
  ...
  #DP:
  RED = RoadEdgeDetector()
  RED.set_enabled(params.get_bool("dp_lateral_road_edge_detection"))
  ...
  while True:
    ...
    if model_output is not None:
      ...
      edge_detected_left, edge_detected_right = RED.get_road_edge_detected(modelv2_send.modelV2.roadEdgeStds, modelv2_send.modelV2.laneLineProbs, sm['carState'].leftBlinker, sm['carState'].rightBlinker)
      DH.update(sm['carState'], sm['carControl'].latActive, lane_change_prob, dp_lat_lane_change_assist_speed, dp_auto_lane_change, edge_detected_left, edge_detected_right)
